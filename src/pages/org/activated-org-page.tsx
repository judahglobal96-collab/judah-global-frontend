...
export default function ActivatedOrgPage() {
  ...
  const subscriptionPrice = formatMoney(
    organization?.subscription_price_cents,
    organization?.subscription_currency || "usd"
  );

  const isSubscriptionActive = useMemo(() => {
    if (subscriptionStatus !== "active") return false;
    if (!organization?.subscription_expires_at) return true;

    const expires = new Date(organization.subscription_expires_at);
    if (Number.isNaN(expires.getTime())) return true;

    return expires.getTime() > Date.now();
  }, [subscriptionStatus, organization?.subscription_expires_at]);
  ...
  if (loading) {
    return (
      <div style={{ color: "#f5f1e8", padding: "8px 0" }}>
      ...
      >
        ...
        >
          <div>
            ...
            >
              Word of mouth outreach just went GLOBAL. {orgName} is active on
              Judah Global. Your organization can submit events, promote key
              gatherings, build visibility, and grow your presence across the
              platform.
            </p>
          </div>
          ...
          >
            <InfoCard label="Org Status" value={orgStatus} highlight />
            <InfoCard
              label="Organization UUID"
              value={organization?.org_uuid || "—"}
              highlight={isSubscriptionActive}
            />
            <InfoCard label="Region" value={subscriptionRegion} />
            <InfoCard label="Price Paid" value={subscriptionPrice} />
            <InfoCard
              label="Expires"
      ...
      >
        ...
        <div style={{ display: "grid", gap: "24px" }}>
          ...
          >
            ...
            >
              Start by submitting your first event
            </h3>
